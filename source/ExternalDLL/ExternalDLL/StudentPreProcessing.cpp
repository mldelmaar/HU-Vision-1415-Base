	...
	for (int x = 0; x < image.getWidth(); x++){
		for (int y = 0; y < image.getHeight(); y++){
			...
			*/
			...
			//5x5
			//rij 1
			sum += image.getPixel(x - 2, y - 2) * -4.0f;
			sum += image.getPixel(x - 1, y - 2) * -1.0f;
			sum += image.getPixel(x, y - 2)		* 0.0f;
			sum += image.getPixel(x + 1, y - 2) * -1.0f;
			sum += image.getPixel(x + 2, y - 2) * -4.0f;

			//rij 2
			sum += image.getPixel(x - 2, y - 1) * -1.0f;
			sum += image.getPixel(x - 1, y - 1) * 2.0f;
			sum += image.getPixel(x , y - 1)	* 3.0f;
			sum += image.getPixel(x + 1, y - 1) * 2.0f;
			sum += image.getPixel(x + 2, y - 1) * -1.0f;

			//rij 3
			sum += image.getPixel(x - 2, y) * 0.0f;
			sum += image.getPixel(x - 1, y) * 3.0f;
			sum += image.getPixel(x, y)		* 4.0f;
			sum += image.getPixel(x + 1, y) * 3.0f;
			sum += image.getPixel(x + 2, y) * 0.0f;
