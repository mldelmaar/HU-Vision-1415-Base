#ifndef _BASETIMER_CPP_
#define _BASETIMER_CPP_
...
/**
  The main constructor of the BaseTimer2 class.
  This initializes everything to zero.
 */
BaseTimer2::BaseTimer2() {
...
*/
unsigned long long BaseTimer2::elapsedMicroSeconds() {
...
*/
unsigned long long BaseTimer2::elapsedMilliSeconds() {
	unsigned long long nTicks = __stop - __start;
 ...
 */
double BaseTimer2::elapsedSeconds() {
	unsigned long long nTicks = __stop - __start;
	return static_cast<double>(nTicks) / static_cast<double>(__nFreq);
}

unsigned long long BaseTimer2::GetPerformanceTicks() {
	LARGE_INTEGER nValue;
	::QueryPerformanceCounter(&nValue);
	return nValue.QuadPart;
}

unsigned long long BaseTimer2::GetPerformanceTicksInSecond() {
	LARGE_INTEGER nFreq;
 ...
 */
bool BaseTimer2::save(std::string filename) {
